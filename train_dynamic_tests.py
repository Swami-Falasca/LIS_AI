...
def train_random_forest(X_train, X_test, y_train, y_test, label_map):
    """Random Forest Classifier"""
    from sklearn.ensemble import RandomForestClassifier
    
    # Appiattisci: (n, 30, 42) -> (n, 1260)
    ...
    return model, results, X_train_flat.shape[1]

def train_xgboost(X_train, X_test, y_train, y_test, label_map):
    """XGBoost Classifier"""
    try:
...
def train_svm(X_train, X_test, y_train, y_test, label_map):
    """Support Vector Machine"""
    from sklearn.svm import SVC
    
    # Appiattisci (usa subset per velocità)
    X_train_flat = X_train.reshape(X_train.shape[0], -1)
    X_test_flat = X_test.reshape(X_test.shape[0], -1)
    
    # Limita dati per velocità
    max_samples = min(1000, len(X_train_flat))
    X_train_flat = X_train_flat[:max_samples]
    y_train = y_train[:max_samples]
    
    print("  Training SVM (con subset dati)...")
    start_time = time.time()
    ...
    results = {
        ...
        'kernel': 'rbf',
        'input_shape': X_train_flat.shape[1],
        'note': f'Trained on {max_samples} samples'
    }
...
def train_mlp(X_train, X_test, y_train, y_test, label_map):
    ...
    for clf_name, clf_func in classifiers:
        ...
        try:
            ...
            if model is not None and results is not None:
                ...
                print(f"  ⏱️  Tempo training: {results['train_time']:.1f}s")
            else:
                print(f"  ⚠️  {clf_name} non disponibile o errore")
